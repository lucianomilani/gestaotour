-- ============================================================
-- TESTE DE CLONAGEM: Clonar manual@teste.com para clone@teste.com
-- ============================================================
-- Se este usuário funcionar, o problema está em algum campo que
-- não estávamos preenchendo exatamente igual.
-- ============================================================

DO $$
DECLARE
    source_email text := 'manual@teste.com';
    target_email text := 'clone@teste.com';
...
BEGIN
    ...
    INSERT INTO auth.identities (
        ...
        gen_random_uuid(),
        new_uid,
        new_uid::text, -- provider_id = user_id para email
        jsonb_build_object(
            'sub', new_uid::text,
            'email', target_email,
